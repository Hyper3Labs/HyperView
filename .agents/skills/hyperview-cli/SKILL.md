...
- Export a Static Space bundle with `hyperview export`.
...
8. Export a Static Space with `hyperview export <workspace-id> --out bundle/`.
...
8. Export a Static Space with `hyperview export <workspace-id> --out bundle/`, or serve that same bundle live with `hyperview serve --from bundle/`.
...
- Extensions define reusable tools/panels; workspace views compose concrete panel instances and layout. In Python launch scripts, register extensions with `hv.launch(..., extensions=[...])` or `session.ui.apply_view(view, extensions=[...])` (or `session.ui.add_extension(...)` on its own) and place panels with `hv.ui.ExtensionPanel(...)`.
...
- For side-by-side embedding comparisons, add scatter panels through `hyperview ui panel add --layout-key ... --reference-panel-id ... --direction right`; the layout key is what makes it a scatter panel.
...
- For demos/spaces that launch HyperView from Python, compose panels with `hv.ui.Horizontal`, `hv.ui.Vertical`, `hv.ui.Tabs`, `hv.ui.Grid`, `hv.ui.Scatter`, `hv.ui.Samples`, `hv.ui.Explorer`, `hv.ui.ExtensionPanel`, and `hv.ui.PanelLayout`; keep extension manifests focused on reusable panel/tool definitions. `hv.ui.Panel(panel_type, id=...)` places any registered panel type when none of the named classes fits.
- Register extensions with the call that applies the view: `hv.launch(..., extensions=[folder_or_shipped_name])` or `session.ui.apply_view(view, extensions=[...])`. An extension panel type does not exist until its extension is registered, and applying a view validates every panel type first.
- Do not pin layout keys as string constants; they carry a content hash that a rebuild invalidates. Use `dataset.find_layout(model=..., provider=..., modality=..., geometry=..., dimension=..., method=...)`, and `dataset.list_layouts()` to see what is available. When one model has both an image-only and a multimodal space in the same dataset, `modality=` is the criterion that tells their layouts apart.
- Use `session.create_collection(sample_ids, name=...)` for a durable, ordered result set a panel can open on, rather than reading a collection id back out of a Samples panel mutation.
- Use `hv.ui.Panel(..., state={...})` for a panel's opening runtime state instead of a `patch_panel_state(..., replace_state=True)` call after `apply_view`.
- An explicit `hv.ui.View` with no Scatter panel can launch from dataset records without an embedding or layout. Only include a Scatter panel when the demo genuinely needs a visualization; Scatter views still require a layout or embedding space.
...
- Author custom panels as `panel.jsx` using the public `window.HyperViewPanelSDK` surface only. HyperView standardizes the data and interaction contract (props, selection, collections, sample results, sibling `updateProps`, panel state), not the panel's visual design. A panel is free to render any JSX, CSS, SVG, Canvas, or WebGL experience that fits its purpose; when it should look like the rest of the workspace, build it from `sdk.components` (`Panel`, `PanelHeader`, `PanelToolbar`, `PanelToolbarButton`, `PanelToolbarIconButton`) instead of hand-rolling chrome in inline CSS.
...
- For public, read-only examples-gallery demos, create a Static Space with `hyperview export <workspace-id> --out bundle/` instead of keeping a Python server awake, then ship it with `hyperview publish bundle/ --to hf:<owner>/<name>`. A bundle is location-independent: copy it to any path inside a containing static site and it resolves its own assets, API, and media from the document URL. Use **Live Space** for the runtime-connected deployment that can run new queries, providers, and workspace mutations.
