...
Redirecting both storage roots at a per-session temporary directory pins every
registry path with them: `get_runtime_config_dir()` derives the config directory
from `datasets_dir.parent`, so workspaces.json, providers.json and jobs.json all
follow the datasets directory here.
"""
