...
>(({ className, inset, children, ...props }, ref) => (
  <DropdownMenuPrimitive.SubTrigger
    ref={ref}
    className={cn(
      "flex h-6 cursor-default select-none items-center gap-1.5 rounded px-1.5 text-[12px] leading-[16px] outline-none focus:bg-accent/70 focus:text-accent-foreground data-[state=open]:bg-accent/70 data-[state=open]:text-accent-foreground data-[disabled]:pointer-events-none data-[disabled]:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-3.5 [&_svg]:shrink-0",
      inset && "pl-8",
      className
    )}
    {...props}
...
>(({ className, sideOffset = 3, ...props }, ref) => (
  <DropdownMenuPrimitive.Portal>
    <DropdownMenuPrimitive.Content
      ...
      className={cn(
        "z-50 max-h-[var(--radix-dropdown-menu-content-available-height)] min-w-[10rem] overflow-y-auto overflow-x-hidden rounded-lg border border-border bg-popover p-1 text-popover-foreground shadow-md",
...
>(({ className, inset, ...props }, ref) => (
  <DropdownMenuPrimitive.Label
    ref={ref}
    className={cn(
      "px-1.5 py-1 text-[11px] leading-4 font-medium text-muted-foreground",
      inset && "pl-8",
      className
    )}
    {...props}
...
>(({ className, ...props }, ref) => (
  <DropdownMenuPrimitive.Separator
    ref={ref}
    className={cn("-mx-1 my-0.5 h-px bg-border/80", className)}
    {...props}
  />
))
...
const DropdownMenuShortcut = ({
  ...
  return (
    <span
      className={cn("ml-auto text-[10px] tracking-[0.06em] text-muted-foreground", className)}
      {...props}
    />
  )
}
